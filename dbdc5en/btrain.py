...
class Trainer():
    ...
    def load_data(self, data, tokenizer, train=False, filter=False):
        if train:
        ...
        for inst in data:
            ...
            if filter:
                if 'id' in inst.keys() and max(label_scores) >= 0.4:
    ...
    def run_process(self):
        ...
        for iter in trange(ITERATIONS, desc='ITER'):
            ...
            if iter >= 0:
                ...
                train_data_out = \
                    self.merge_label_scores(self.masked_ref_data, self.gen_out, iter=iter, total_iter=5)
